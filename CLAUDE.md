...
| Request stuck in processing | URLs summarize only through the graph: start with the URL-flow facade `app/adapters/content/graph_url_processor.py` (`GraphURLProcessor.handle_url_flow`), the graph spine `app/application/graphs/summarize/` (`graph.py` + `nodes/`, especially `ingest`/`extract`/`persist`/`notify`), `app/adapters/content/platform_extraction/lifecycle.py`, `app/db/models/core.py::RequestProcessingJob`, and `docs/reference/troubleshooting.md#request-stuck-in-processing`; keep correlation IDs intact across any repair (`thread_id == correlation_id`). |
| Attachment or forwarded-message summary | These do **not** run the graph -- it is the sole summarize path for URLs only. Attachments go through `app/adapters/attachment/_attachment_llm.py` and `_attachment_content.py`, forwarded Telegram messages through `app/adapters/telegram/forward_summarizer.py`, both calling `LLMResponseWorkflow` (`app/application/services/summarization/llm_response_workflow.py`) directly. That is why `llm_calls` still carries `auto_backfill` and `repair_loop` rows alongside `graph_node`. |
...
| Gated academic PDF recovery | When the academic extractor's cookie-less httpx PDF download is blocked (paywall/403/non-PDF), it re-fetches the PDF through the **CloakBrowser** stealth session that already cleared Cloudflare for the landing page (the deterministic httpx download can't carry `cf_clearance`). Tiered: tier 1 = the known/anchor PDF URL via `CloakBrowserProvider.fetch_pdf` (cheap, no LLM); tier 2 = autonomous download-control click via `CloakBrowserProvider.download_pdf_via_controls` + `app/adapters/academic/agentic_pdf.py::AgenticPdfDownloader` (opt-in, host-allowlisted). Entry: `AcademicPlatformExtractor._recover_via_browser_pdf` (`app/adapters/academic/platform_extractor.py`), runs after the httpx PDF attempt and before the OA-metadata fallback. Gated by `ACADEMIC_BROWSER_PDF_RECOVERY_ENABLED` / `ACADEMIC_AGENTIC_PDF_DOWNLOAD_ENABLED` + `ACADEMIC_AGENTIC_PDF_HOST_ALLOWLIST` **and** requires the CloakBrowser sidecar running (`with-scrapers` profile, reachable `SCRAPER_CLOAKBROWSER_URL`); intentionally **independent of** the scraper-chain `SCRAPER_CLOAKBROWSER_ENABLED` flag (built directly in `_build_academic_browser_pdf`, not via the chain factory), so a deployment can keep CloakBrowser off as a general rung. No-ops otherwise. Distinct from the OA-metadata fallback (open scholarly APIs) — this scrapes the publisher's own PDF. |
...
| On-disk git mirroring (git backup) | Engine and service: `app/adapters/git_backup/` (`mirror_service.py` = `GitMirrorService`, `repository.py` = `GitMirrorRepository`). Config: `app/config/git_backup.py` (`GitBackupConfig`). DB model: `app/db/models/git_backup.py` (`GitMirror`). Scheduled Taskiq job: `app/tasks/git_backup_sync.py` (task name `ratatoskr.git_backup.sync`, Redis-locked, cron `GIT_BACKUP_SYNC_CRON`). Telegram commands `/mirror` and `/mirrors`: `app/adapters/telegram/command_handlers/git_mirror_handler.py`. REST endpoints `GET/POST/DELETE /v1/git-mirrors`: `app/api/routers/git_mirrors.py`. **Distinct from the GitHub API-based metadata ingestion** (`app/adapters/github/`, `app/tasks/github_sync.py`, plus the opt-in star-list filing pass `app/tasks/star_list_filing.py` / `ratatoskr.github.file_unfiled`, which files already-starred repositories that carry no list, and the Telegram `/star` command `app/adapters/telegram/command_handlers/star_repository_handler.py` composed via `app/di/add_repository.py`) which never clones to disk — that path fetches repo metadata and indexes it in PostgreSQL + Qdrant. Git backup performs actual `git clone --mirror` of full history to `GIT_BACKUP_DATA_PATH` and reuses `GITHUB_TOKEN_ENCRYPTION_KEY` for authenticated clones. |
| AI account backup | Backup clients and browser-auth adapters: `app/adapters/ai_backup/`, `app/adapters/content/browser_auth/`. Config: `app/config/ai_backup.py` (`AiBackupConfig`). DB model: `app/db/models/ai_backup.py` (`AiAccountBackup`; enums `AiBackupService`, `AiBackupStatus`). Scheduled Taskiq job: `app/tasks/ai_backup_sync.py` (task name `ratatoskr.ai_backup.sync`, Redis-locked, cron `AI_BACKUP_SYNC_CRON`). REST endpoints `GET/POST /v1/ai-backups`: `app/api/routers/ai_backups.py`. Double-gated: `AI_BACKUP_ENABLED` plus the per-service switch (`AI_BACKUP_CHATGPT_ENABLED` / `AI_BACKUP_CLAUDE_ENABLED`). Requires the CloakBrowser sidecar (`with-scrapers` profile). When `AI_BACKUP_CLAUDE_COMPLIANCE_KEY` is set the Claude path uses the sanctioned Anthropic Compliance API instead of the stealth scrape. Design: `docs/explanation/ai-account-backup.md`. |
...
| `langgraph-summarize-loop` | summarize-graph node walk + retry / repair-loop / `attempt_trigger` debugging (the graph is the sole summarize path **for URLs**; `attempt_trigger='graph_node'`) |
...
make pi-deploy-all                    # AI re-auth stack + app services + pg-backup
...
| `AI_BACKUP_ENABLED`, `AI_BACKUP_CHATGPT_ENABLED`, `AI_BACKUP_CLAUDE_ENABLED`, `AI_BACKUP_SYNC_CRON`, `AI_BACKUP_DATA_PATH` | AI account backup subsystem. Master switch (default `false`) double-gated with per-service flags (both default `false`), Taskiq cron schedule (default `0 5 * * *` UTC), and local filesystem path for backed-up data (default `/data/ai-backups`). Full variable set in `docs/reference/environment-variables.md`. |
| `OTEL_ENABLED`, `OTEL_TRACES_EXPORTER`, `OTEL_EXPORTER_OTLP_ENDPOINT`, `OTEL_FILE_EXPORTER_PATH` | OpenTelemetry tracing. **Two different defaults, on purpose:** Compose ships `OTEL_ENABLED=true` for the five service processes, while `OtelConfig.enabled` stays `False` so a bare CLI run or test traces nothing. The collector is the `tempo` service in the **primary** Compose file under the `with-monitoring` profile -- it used to exist only in `docker-compose.monitoring.yml` on a separate network, so the default endpoint named a host nothing created. Span exporter `otlp` (Tempo, default) / `console` / `file` (swap needs no code change); OTLP endpoint (default `http://tempo:4317`); file-exporter path for ad-hoc DuckDB/Polars analysis -- the no-collector option for a RAM-tight host. 100% sampling is hard-wired (`OTEL_SAMPLE_RATIO` is a documented no-op). Flushing is bounded by `SPAN_FLUSH_TIMEOUT_SEC` (5 s) so an absent collector cannot spend a container's whole stop grace period. Full set + behavior in `docs/reference/environment-variables.md#observability--opentelemetry-tracing`. |
