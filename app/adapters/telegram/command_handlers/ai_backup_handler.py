"""Telegram command handlers for /ai_backup and /ai_backups (read-only status).
...
Session ingest (Mode A) is REST-only (POST /v1/ai-backups/{service}/session) --
the session blob contains live cookies and must not transit Telegram's non-E2E
chat infrastructure (stored on Telegram servers, shown in notification previews).
"""
