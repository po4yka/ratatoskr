...
def source_item_from_record(
    ...
    *,
    ...
    return AggregationSourceItem(
        bundleId=int(record.get("aggregation_session_id") or fallback_session_id or 0),
        sourceItemId=str(record.get("source_item_id") or ""),
        itemId=record.get("id"),
        position=int(record.get("position") or 0),
        originalUrl=original_url,
        normalizedUrl=normalized_url,
        sourceKind=str(record.get("source_kind") or SourceKind.UNKNOWN.value),
        extractionStatus=str(record.get("status") or "unknown"),
        title=title,
        domain=str(domain).strip() if domain else None,
        author=str(author).strip() if author else None,
        publishedAt=str(published_at).strip() if published_at else None,
        errorCode=record.get("failure_code"),
        errorMessage=record.get("failure_message"),
        requestId=record.get("request_id"),
        crawlResultId=record.get("crawl_result_id"),
        summaryId=None if deleted else record.get("summary_id"),
        duplicateOfItemId=record.get("duplicate_of_item_id"),
        deleted=deleted,
        metadata={
            key: value
            for key, value in {
...
def source_item_from_extraction_result(
    ...
    *,
    ...
    return AggregationSourceItem(
        ...
        publishedAt=str(document_metadata.get("published_at") or "").strip() or None,
        errorCode=failure.code if failure is not None else None,
        errorMessage=failure.message if failure is not None else None,
