...
class CocoIndexConfig(BaseModel):
    ...
    enabled: bool = Field(
        default=False,
        validation_alias="RATATOSKR_COCOINDEX_ENABLED",
        description="Enable CocoIndex live incremental sync from summaries to Qdrant",
    )
    dsn_override: str | None = Field(
        default=None,
        validation_alias="RATATOSKR_COCOINDEX_DSN",
        description="Override DATABASE_URL for CocoIndex's psycopg3 connection (strips asyncpg prefix automatically)",
    )
    poll_interval_sec: int = Field(
        default=30,
        validation_alias="RATATOSKR_COCOINDEX_POLL_INTERVAL_SEC",
        description="Seconds between ordinal watermark polls when LISTEN/NOTIFY is idle",
    )
    listen_notify_channel: str = Field(
        default="ratatoskr_summaries_changed",
        validation_alias="RATATOSKR_COCOINDEX_LISTEN_CHANNEL",
        description="Postgres LISTEN/NOTIFY channel name for live change detection",
    )
    batch_size: int = Field(
        default=32,
        validation_alias="RATATOSKR_COCOINDEX_BATCH_SIZE",
        description="Number of rows per CocoIndex processing batch",
    )
    pool_max: int = Field(
        default=4,
        validation_alias="RATATOSKR_COCOINDEX_POOL_MAX",
        description="Max psycopg3 connections for CocoIndex (counts against Postgres max_connections)",
    )
