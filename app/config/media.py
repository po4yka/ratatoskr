...
class YouTubeConfig(BaseModel):
    ...
    def _parse_subtitle_languages(cls, value: Any) -> list[str]:
        ...
        if isinstance(value, str):
            parsed = [lang.strip() for lang in value.split(",") if lang.strip()]
            if not parsed:
                msg = "subtitle_languages is required (no code default); set it in ratatoskr.yaml"
    ...
    def _parse_int_fields(cls, value: Any, info: ValidationInfo) -> int:
        if value in (None, ""):
            msg = f"{info.field_name} is required (no code default); set it in ratatoskr.yaml"
            raise ValueError(msg)
        try:
    ...
    def _validate_preferred_quality(cls, value: Any) -> str:
        if value in (None, ""):
            msg = "preferred_quality is required (no code default); set it in ratatoskr.yaml"
            raise ValueError(msg)
