...
This module lives in ``app.core`` so that application-layer services can
import it without violating the ``application-no-outward`` layering contract.

``app.adapters.llm.usage_budget`` re-exports everything from here so that
adapter-side callers (e.g. ``app.api.services.admin_read_service``) are
unchanged.
"""
