...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column(
            "allowed_domains_json", postgresql.JSONB(astext_type=sa.Text()), nullable=True
        ),
        sa.Column(
            "status",
        ...
        sa.Column(
            "screenshots_json", postgresql.JSONB(astext_type=sa.Text()), nullable=True
        ),
    ...
    op.create_index(
        "ix_webwright_runs_correlation_id", "webwright_runs", ["correlation_id"]
    )
    ...
    op.create_table(
        ...
        sa.PrimaryKeyConstraint("id"),
        sa.UniqueConstraint(
            "user_id", "domain", name="uq_user_browser_sessions_user_domain"
        ),
    )
    op.create_index(
        "ix_user_browser_sessions_user_id", "user_browser_sessions", ["user_id"]
    )
