"""Infrastructure-layer persistence for durable request-processing jobs.

This module owns ``RequestProcessingJobRepository`` and the closely-coupled
``LeasedRequestJob`` value object.  Both are pure infrastructure: they depend
only on SQLAlchemy, app.db, and app.core — never on app.api.

The API background layer (``app.api.background.durable_jobs``) re-exports all
public names from here so existing API-layer importers need no changes.
"""
