"""Taskiq task: periodic delta-scan of the x_bookmarks wiki library directory.

Mirrors the reconcile-style pattern of ``app/tasks/x_bookmarks_sync.py`` (the
...
``_wiki_sync_body`` handles the ``cfg.x_bookmarks.enabled`` gate and delegates
to :class:`XWikiSyncService`. The service walks
``cfg.x_bookmarks.library_path``, content-hashes each ``*.md`` page, embeds
changed pages into the shared Qdrant collection as
``entity_type="x_wiki"`` points, and hard-deletes orphan paths.

The wiki has no Postgres mirror; Qdrant is its sole persistence beyond the
source filesystem (see ``docs/explanation/x-bookmarks-integration.md``).
"""
...
_X_WIKI_SYNC_LOCK_KEY = "task_lock:x_wiki_sync"
...
# mid-run.
_X_WIKI_SYNC_LOCK_TTL = 1800


@broker.task(task_name="ratatoskr.x.sync_wiki")
async def sync_x_wiki(
    cfg: AppConfig = TaskiqDepends(get_app_config),
    db: Database = TaskiqDepends(get_db),
) -> WikiSyncSummary:
    """Run one delta-scan pass over the x_bookmarks wiki library directory."""
    redis_client = await get_redis(cfg)
    async with RedisDistributedLock(
        redis_client,
        _X_WIKI_SYNC_LOCK_KEY,
        _X_WIKI_SYNC_LOCK_TTL,
    ) as acquired:
        if not acquired:
            logger.info(
                "x_wiki_sync_skipped_lock_held",
                extra={"key": _X_WIKI_SYNC_LOCK_KEY},
            )
...
async def _wiki_sync_body(cfg: AppConfig, db: Database) -> WikiSyncSummary:
    if not cfg.x_bookmarks.enabled:
        logger.info(
            "x_wiki_sync_disabled",
            extra={"library_path": cfg.x_bookmarks.library_path},
        )
        return WikiSyncSummary()

    runtime = build_x_wiki_sync_task_runtime(cfg, db)
    logger.info(
        "x_wiki_sync_starting",
        extra={"library_path": cfg.x_bookmarks.library_path},
    )
    summary: WikiSyncSummary = await runtime.service.sync()
    logger.info(
        "x_wiki_sync_complete",
        extra={
