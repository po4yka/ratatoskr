...
| `maintenance` | `app/adapters/git_backup/maintenance.py` | `RepositoryMaintenance` runs post-sync maintenance on each bare clone. Three strategies: `gc-auto` (`git gc --auto`), `geometric` (`git repack --geometric=2 -d`), and `none`. Optionally writes a commit-graph (`git commit-graph write --reachable`) after every sync. A periodic full repack (`git repack -a -d`) runs 7 or 30 days apart when `GIT_BACKUP_FULL_REPACK_INTERVAL` is `weekly` or `monthly`, paced by a marker file at the root of the mirror store. Maintenance runs in `asyncio.to_thread` so it does not block the event loop. |
...
| `clone_url` | varchar(1000) | no | Canonical clone URL stored without credentials |
...
Manual and `extra_repos` mirrors are cloned unauthenticated after the same
credential and network-target guards are applied.
...
`register_sync_and_check_repack` is called **once per sync run** (in `perform_sync` after all outcomes are persisted), not once per successful repo. Cadence state lives in a `.ratatoskr-last-full-repack` marker file at the root of `GIT_BACKUP_DATA_PATH`, whose mtime is the last repack time. An in-memory counter was used before and never fired: the Taskiq task rebuilds the service on every run, so the counter reset to 0 each night and never reached 7 or 30. The marker is stamped when the timer is first armed and when a repack is attempted, never on a run that is not yet due — re-stamping on a quiet run would push the deadline forward forever.
