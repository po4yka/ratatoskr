...
5. New `app/infrastructure/checkpointing/runtime.py` `CheckpointerRuntime` (template `app/infrastructure/cocoindex/runtime.py`): build a dedicated psycopg3 `AsyncConnectionPool` (autocommit, `row_factory=dict_row`, search_path = `langgraph`), wrap in `AsyncPostgresSaver`, `await saver.setup()`, expose `start()/stop()`. Strip the asyncpg DSN prefix exactly as runtime.py:44. **Must NOT route through `Database`.**
6. Hook start/stop into both lifespans: `app/api/main.py` (near CocoIndex block ~186–211, same failure-isolation try/except + finally-stop) and `bot.py main()` (after `db.migrate()`, shutdown in finally ~85–93). Gate on `cfg.langgraph_checkpoint.enabled`.
...
9. Document new env vars in `docs/reference/environment-variables.md` and reconcile the pool-size figure in `docs/cocoindex.md` connection-budget table (73–81).

**Files:** `app/db/models/core.py`, `app/config/{settings,__init__}.py`, `bot.py`, `app/api/main.py`, `app/tasks/scheduler.py`, `ops/docker/docker-compose.yml`, `docs/cocoindex.md`, `docs/reference/environment-variables.md`, `pyproject.toml`, `uv.lock`.
...
12. Compose in `app/di/retrieval.py` (or extend `di/search.py`): single adapter injected into API search service, MCP context, git_mirrors router. This is the seam ADR-0010 needs for the future `ground` node — do NOT build retrieval into CocoIndex flows.
...
9. Demote CocoIndex + reconcile to backfill/repair (doc/comment only): fast-path = freshness, CocoIndex/reconciler = convergence. **Verify payload byte-compat** with `flow.py::_build_summary_payload` (entity_type/summary_id/request_id/user_scope/environment/language/...) so `query()` and the reconciler see no drift/duplicate.
...
| R6 | **Pool exhaustion** — ADR-0004 (max=5) vs cocoindex doc (max=10) disagreement, plus two drivers in one process. | Reconcile to a single documented value in T2; verify Postgres `max_connections` headroom across all pools; checkpointer pool isolated from `Database`. |
