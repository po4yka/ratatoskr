...
- `crawl_results.content_text` is the normalized durable source body used by graph resume and Reader; `crawl_results.id` is the source artifact handle. Raw provider fields use `RETENTION_RAW_EXTRACTED_CONTENT_DAYS`, while this normalized body has its own `RETENTION_READER_SOURCE_CONTENT_DAYS` policy and is retained indefinitely by default (`0`).
- `ratatoskr.source_content.reconcile` repairs completed legacy rows that lack this body. It materializes cleaned local markdown/HTML/request text first, advances a Redis cursor so permanently failing rows cannot starve the backlog, and spends at most `SOURCE_CONTENT_RECONCILE_NETWORK_LIMIT` network attempts per run. Preview the exact backlog without writes with `python -m app.cli.reconcile_source_content`; repair one bounded batch with `--apply`.
