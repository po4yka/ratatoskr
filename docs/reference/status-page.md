...
| Scraper / extraction | Bot and worker runtime telemetry reports the latest non-policy-blocked chain result. After a process restart clears that telemetry, the latest persisted terminal URL request supplies a bounded fallback; unrelated summarization failures are ignored. With reachable exporters and database, no recent workload is reported as operational idle rather than a telemetry failure. No synthetic crawl is performed. |
| AI summarization | The worker's allowlisted OpenRouter circuit-breaker updates are authoritative. After a process restart clears them, the latest persisted OpenRouter call supplies a bounded fallback. With a reachable worker exporter and database, no recent workload is reported as operational idle. One failed model degrades the runtime fallback chain, while all fresh observed models open is an outage. Other providers remain `unknown` until they expose an equivalent live signal. |
...
signal can use a persisted outcome only while the corresponding process
exporter remains reachable. Fresh terminal failures remain degraded; stale or
absent workload is operational idle when the exporter and database are
reachable. Unavailable exporters, unavailable persisted telemetry, and invalid
