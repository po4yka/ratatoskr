...
8. If adding a repository with `mode=star` returns 200 but `lists_applied` is empty and a warning names a missing scope, the token predates the `user` scope. Starring works on `repo`, so the repository is starred but unfiled. Ask the user to reconnect (`POST /v1/auth/github/pat` with a token carrying `user`, or restart the device flow, which now requests `read:user repo user`), then file it with `PUT /v1/star-lists/repositories/{repository_id}`. Do not re-add the repository: `mode=star` is idempotent but would re-run the suggester.
9. If added repositories are consistently left unfiled with no scope warning, the suggester is inconclusive rather than failing. Check that Qdrant is reachable (without it there are no neighbours to vote with) and that `GITHUB_STAR_LIST_SUGGEST_LLM_FALLBACK` is on; lowering `GITHUB_STAR_LIST_SUGGEST_MIN_SCORE` accepts weaker matches at the cost of more wrong filings.
10. If the Taskiq job dead-lettered, inspect `taskiq_failed_jobs`, fix the upstream GitHub/LLM/config issue, then replay using `python -m app.cli.requeue_failed_task <id>`.
