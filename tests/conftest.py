...
def deterministic_dns(request, monkeypatch):
    ...
    Layering with `enforce_no_network`, which patches the same function: this
    fixture makes lookups *deterministic* for every test, that one makes them
    *fail loudly* for `no_network`-marked tests. It is defined first on purpose,
    so `enforce_no_network` patches last and its guard wins wherever both apply
    -- a marked test must see `BlockedNetworkError`, not a table answer. Keep
    that order if either fixture moves.
    """
