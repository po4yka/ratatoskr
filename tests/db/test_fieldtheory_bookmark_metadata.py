"""Schema-only tests for the ``fieldtheory_bookmark_metadata`` model.
...
def _table() -> Table:
    ...
    """
    return cast("Table", FieldTheoryBookmarkMetadata.__table__)


def test_fieldtheory_bookmark_metadata_registered_in_core_models() -> None:
    assert FieldTheoryBookmarkMetadata in CORE_MODELS
    assert FieldTheoryBookmarkMetadata in ALL_MODELS


def test_fieldtheory_bookmark_metadata_table_name() -> None:
    assert FieldTheoryBookmarkMetadata.__tablename__ == "fieldtheory_bookmark_metadata"


def test_fieldtheory_bookmark_metadata_columns_match_design() -> None:
    columns = {col.name for col in FieldTheoryBookmarkMetadata.__table__.columns}
    assert columns == {
        "request_id",
        "fieldtheory_id",
...
def test_fieldtheory_bookmark_metadata_request_id_is_pk_and_fk() -> None:
    table = _table()
...
def test_fieldtheory_bookmark_metadata_nullability() -> None:
    table = _table()
    assert table.c.fieldtheory_id.nullable is False
    assert table.c.fieldtheory_category.nullable is False
...
def test_fieldtheory_bookmark_metadata_has_no_lifecycle_columns() -> None:
...
def test_fieldtheory_bookmark_metadata_indexes_present() -> None:
    table = _table()
...
def test_fieldtheory_id_index_is_unique() -> None:
    table = _table()
    unique_index = next(
        idx
        for idx in table.indexes
        if idx.name == "ix_fieldtheory_bookmark_metadata_fieldtheory_id"
    )
    assert unique_index.unique is True
    assert [col.name for col in unique_index.columns] == ["fieldtheory_id"]
...
def test_tweet_text_tsv_index_uses_gin() -> None:
    ...
    tsv_index = next(
        idx
        for idx in table.indexes
        if idx.name == "ix_fieldtheory_bookmark_metadata_tweet_text_tsv"
    )
...
def test_category_check_constraint_pins_v2_vocabulary() -> None:
    ...
    matched = [c for c in checks if c.name == "ck_fieldtheory_bookmark_metadata_category"]
    assert len(matched) == 1, (
        "expected one CHECK constraint named ck_fieldtheory_bookmark_metadata_category"
    )
...
def test_index_definitions_use_real_index_objects() -> None:
    ...
    assert "ix_fieldtheory_bookmark_metadata_category" in explicit_indexes
    category_index = next(
        idx for idx in table.indexes if idx.name == "ix_fieldtheory_bookmark_metadata_category"
    )
    assert isinstance(category_index, Index)
    assert [col.name for col in category_index.columns] == ["fieldtheory_category"]
...
def test_synced_at_has_default() -> None:
    ...
    column = cast(
        "object",
        FieldTheoryBookmarkMetadata.__table__.c.synced_at,
    )
