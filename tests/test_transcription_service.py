...
async def test_max_duration_guard_refuses_long_media() -> None:
    ...
    with patch("app.adapters.transcription.service.probe_duration_sec", return_value=300.0):
...
async def test_happy_path_returns_plain_text_and_sentences() -> None:
    ...
    with (
        ...
        patch.object(TranscriptionService, "_get_engine", AsyncMock(return_value=engine)),
    ):
        result = await svc.transcribe_media_path(
            Path("/tmp/audio.mp3"),
            progress_callback=lambda stage, status, progress, message: progress_events.append(
...
async def test_diarization_disabled_when_options_override_false() -> None:
    ...
    with (
        patch("app.adapters.transcription.service.probe_duration_sec", return_value=5.0),
        patch(
            "app.adapters.transcription.service.decode_to_pcm",
...
async def test_speed_override_passed_to_engine() -> None:
    ...
    with (
        patch("app.adapters.transcription.service.probe_duration_sec", return_value=3.0),
        patch(
            "app.adapters.transcription.service.decode_to_pcm",
            return_value=np.zeros(8000, dtype=np.float32),
        ) as decode_mock,
...
async def test_empty_recognizer_output_returns_empty_text() -> None:
    ...
    with (
        patch("app.adapters.transcription.service.probe_duration_sec", return_value=2.0),
        patch(
            "app.adapters.transcription.service.decode_to_pcm",
            return_value=np.zeros(8000, dtype=np.float32),
        ),
